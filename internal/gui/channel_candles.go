...
// CandlesChannelPanel manages candles channel configuration
type CandlesChannelPanel struct {
	logger           *zap.Logger
...
func NewCandlesChannelPanel(logger *zap.Logger, configManager *config.ConfigManager, exchange string) *CandlesChannelPanel {
	panel := &CandlesChannelPanel{
		logger:          logger,
...
func (p *CandlesChannelPanel) SetOnStateChange(fn func()) {
	p.onStateChange = fn
}

func (p *CandlesChannelPanel) SetLimitChecker(fn func(delta int) bool) {
	p.limitChecker = fn
}

func (p *CandlesChannelPanel) Build() fyne.CanvasObject {
	p.enableCheck = widget.NewCheck("Enable Candles Channel", func(checked bool) {
		p.enabled = checked
		if checked {
			p.symbolList.Enable()
			p.timeframeSelect.Enable()
		} else {
			p.symbolList.Disable()
			p.timeframeSelect.Disable()
		}
	...
	timeframes := []string{"1m", "5m", "15m", "30m", "1h", "3h", "6h", "12h", "1D", "1W", "14D", "1M"}
	p.timeframeSelect = widget.NewSelect(timeframes, func(value string) {
		p.timeframe = value
		if p.updating {
			return
		}
	...
	p.timeframeSelect.SetSelected("1m")
	p.timeframeSelect.Disable()

	infoLabel := widget.NewLabel("Candles channel provides OHLC (Open-High-Low-Close) data.")

	configForm := widget.NewForm(
		widget.NewFormItem("Timeframe", p.timeframeSelect),
	)

	symbolScroll := container.NewVScroll(p.symbolList)
	symbolScroll.SetMinSize(fyne.NewSize(400, 350))
...
func (p *CandlesChannelPanel) loadAvailableSymbols() {
...
func (p *CandlesChannelPanel) filterSymbols(searchText string) {
	if p.symbolList == nil {
		return
	}
...
func (p *CandlesChannelPanel) GetSubscriptions() []ChannelSubscription {
	if !p.enabled {
		return []ChannelSubscription{}
	}
	subs := []ChannelSubscription{}
	for symbol := range p.selectedSymbols {
		key := fmt.Sprintf("trade:%s:%s", p.timeframe, symbol)
...
func (p *CandlesChannelPanel) GetSubscriptionCount() int {
	if !p.enabled {
		return 0
	}
...
func (p *CandlesChannelPanel) LoadState(uiState *config.UIState) {
	if uiState == nil || uiState.ChannelStates == nil {
		return
	}
	if channelState, ok := uiState.ChannelStates["candles"].(map[string]interface{}); ok {
		...
		if timeframe, ok := channelState["timeframe"].(string); ok {
			p.timeframe = timeframe
			if p.timeframeSelect != nil {
				p.updating = true
				p.timeframeSelect.SetSelected(timeframe)
				p.updating = false
			}
		}
...
func (p *CandlesChannelPanel) SaveState(uiState *config.UIState) {
	...
	uiState.ChannelStates["candles"] = map[string]interface{}{
		"enabled":          p.enabled,
...
func (p *CandlesChannelPanel) Reset() {
	p.enabled = false
	p.selectedSymbols = make(map[string]bool)
	p.timeframe = "1m"
...
func (p *CandlesChannelPanel) ReloadSymbols() {
...
func (p *CandlesChannelPanel) notifyStateChange() {
	if p.onStateChange != nil {
		p.onStateChange()
	}
}

func (p *CandlesChannelPanel) persistState() {
	...
	if err := p.configManager.SaveState(); err != nil {
		p.logger.Warn("failed to persist candles channel state", zap.Error(err))
	}
}

func (p *CandlesChannelPanel) currentActualSymbols() []string {
...
func (p *CandlesChannelPanel) currentDisplaySelection() []string {
